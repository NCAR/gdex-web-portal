...
class DatasetRequestForm(forms.Form):
    ...
    gindex = forms.ChoiceField(required=True, choices=[], label="gindex", help_text="Group Index from dsrqst control (default 0)")
    ...
    def __init__(self, *args, **kwargs):
        ...
        dsid = self.initial.get('dsid', None)
        self.fields['gindex'].choices = self.get_gindex_choices(dsid) if dsid else [(0, "0 - Default group index")]

    def get_gindex_choices(self, dsid):
        conn, cursor = init_connection_new()
        cursor.execute("SELECT gindex FROM rcrqst WHERE dsid = %s AND (rqsttype = 'S' OR rqsttype = 'T')", (dsid,))
