    ...
    var locationData = {
        'North America': {
            ...
            'Mexico': [
                ...
                'Coahuila','Jalisco','Nuevo León','Oaxaca',
        ...
        'South America': {
            'Argentina': ['Buenos Aires','Córdoba','Mendoza','Patagonia','Santa Fe','Tierra del Fuego'],
            'Bolivia':   ['Altiplano','Cochabamba','La Paz','Santa Cruz'],
            'Brazil':    ['Amazonas','Bahia','Ceará','Mato Grosso','Minas Gerais','Pará','Rio de Janeiro','São Paulo'],
            'Chile':     ['Antofagasta','Atacama','Los Lagos','Magallanes','Tarapacá','Valparaíso'],
            'Colombia':  ['Bogotá','Cundinamarca','Valle del Cauca'],
            'Ecuador':   ['Galápagos','Guayas','Pichincha'],
            'Peru':      ['Amazonas','Andes','Lima'],
            'Venezuela': ['Andes','Llanos','Maracaibo']
        },
        'Europe': {
            'France':   ['Alsace','Brittany','Île-de-France','Normandy','Occitanie','Provence'],
            'Germany':  ['Baden-Württemberg','Bavaria','Berlin','Brandenburg','Hamburg','North Rhine-Westphalia','Saxony'],
            'Iceland':  [],
            'Italy':    ['Lombardy','Sicily','Sardinia','Trentino','Tuscany'],
            'Norway':   ['Finnmark','Nordland','Oslo','Troms','Vestland'],
            'Russia (European)': ['Kola Peninsula','Moscow Oblast','Saint Petersburg','Ural Region'],
            'Spain':    ['Andalusia','Basque Country','Catalonia','Madrid','Valencia'],
            'Sweden':   ['Lapland','Skåne','Stockholm','Västra Götaland'],
            'Switzerland': ['Bern','Geneva','Graubünden','Valais','Zurich'],
            'United Kingdom': ['England','Northern Ireland','Scotland','Wales']
        },
        'Asia': {
            'China':      ['Beijing','Guangdong','Heilongjiang','Inner Mongolia','Qinghai','Shandong','Shanghai','Sichuan','Tibet','Xinjiang'],
            'India':      ['Assam','Gujarat','Himachal Pradesh','Jammu & Kashmir','Kerala','Maharashtra','Rajasthan','Tamil Nadu','Uttarakhand'],
            'Japan':      ['Hokkaido','Honshu','Kyushu','Okinawa','Shikoku'],
            'Kazakhstan': ['Akmola','East Kazakhstan','North Kazakhstan','West Kazakhstan'],
            'Mongolia':   [],
            'Russia (Asian)': ['Kamchatka','Sakha Republic','Siberia','Yakutia'],
            'South Korea': [],
            'Taiwan':     []
        },
        'Africa': {
            'Algeria':      [],
            'Egypt':        [],
            'Ethiopia':     ['Addis Ababa','Afar','Amhara','Oromia','Tigray'],
            'Kenya':        ['Central','Coast','Nairobi','North Eastern','Rift Valley'],
            'Libya':        [],
            'Mali':         [],
            'Morocco':      [],
            'Nigeria':      ['Abuja','Kaduna','Kano','Lagos','Rivers'],
            'South Africa': ['Eastern Cape','Gauteng','KwaZulu-Natal','Limpopo','Western Cape'],
            'Sudan':        [],
            'Tanzania':     []
        },
        'Oceania': {
            ...
            'Fiji':            [],
            'New Zealand':     ['Auckland','Canterbury','Otago','Waikato','Wellington'],
            'Papua New Guinea': []
        },
        'Polar Regions': {
            'Antarctica': [],
        ...
        'Ocean Basins': {
            'Atlantic Ocean': ['Caribbean Sea','Gulf of Mexico','Mediterranean Sea','North Atlantic','South Atlantic'],
            'Pacific Ocean':  ['East Pacific','North Pacific','South Pacific','Tropical Pacific','West Pacific'],
            'Indian Ocean':   ['Arabian Sea','Bay of Bengal','Southern Indian Ocean'],
            'Arctic Ocean':   [],
            'Southern Ocean': ['Amundsen Sea','Ross Sea','Weddell Sea']
        }
    };

    (function () {
        var contEl   = document.getElementById('loc-continent');
        var countEl  = document.getElementById('loc-country');
        var stateEl  = document.getElementById('loc-state');
        var stateRow = document.getElementById('loc-state-row');
        if (!contEl) return;

        Object.keys(locationData).forEach(function (cont) {
            var opt = document.createElement('option');
            opt.value = cont; opt.textContent = cont;
            contEl.appendChild(opt);
        });
        ...
        function removeLocChip() {
            groupSelections['loc'] = [];
            renderGroupChip('loc');
        }

        function addLocChip(label) {
            groupSelections['loc'] = [label];
            renderGroupChip('loc');
        }

        function resetLocationSelects() {
            contEl.value = '';
            fillSelect(countEl, [], 'Select country…');
            countEl.disabled = true;
            fillSelect(stateEl, [], 'Select state…');
            stateEl.disabled = true;
            stateRow.style.display = '';
        }
        window._resetLocationSelects = resetLocationSelects;
